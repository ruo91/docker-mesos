...
ZK_LEADER_PORT="3888"
ZK_FOLLOW_PORT="2888"
ZK_SERVER_1="172.17.2.5"
ZK_SERVER_2="172.17.2.6"
ZK_SERVER_3="172.17.2.7"
...
MESOS_MARATHON_LOGS="/tmp/mesos-marathon.log"
...
MESOS_MARATHON_PID="$(ps -e | grep 'java' | awk '{ printf $1 "\n" }')"
...
function f_zk_manual {
  ...
  for ((i = 1; i < $ZK_SERVER_NUMBER+1; i++)); do
      ...
      echo -ne "\033[33m- ex) 172.17.2.5 \033[0m \n"
      echo -ne "\033[33m- Input: \033[0m"
      read ZK_SERVER
      echo
...
function f_mesos_master_manual {
  ...
  echo -ne "\033[33m- ex) 172.17.2.5:2181,172.17.2.6:2181,172.17.2.7:2181 \033[0m \n"
  echo -ne "\033[33m- Input: \033[0m"
  read ZK_SERVERS
  echo
...
function f_mesos_marathon {
  if [ "$PIPEWORK_IP" == "$MESOS_MARATHON_IP" ]; then
      # Start Mesos Marathon
      echo "Start Mesos Marathon..."  && sleep 1
      $MESOS_MARATHON_BINARY \
      --master $ZK_SERVER_1:$ZK_PORT,$ZK_SERVER_2:$ZK_PORT,$ZK_SERVER_3:$ZK_PORT/$ZK_ZNODE_PATH \
      --zk zk://$ZK_SERVER_1:$ZK_PORT,$ZK_SERVER_2:$ZK_PORT,$ZK_SERVER_3:$ZK_PORT/$ZK_MARATHON_ZNODE_PATH \
      > $MESOS_MARATHON_LOGS 2>&1 &
      echo "done"
...
function f_mesos_marathon_manual {
  echo -ne "\033[33m- ZooKeeper Servers \033[0m \n"
  echo -ne "\033[33m- ex) 172.17.2.5:2181,172.17.2.6:2181,172.17.2.7:2181 \033[0m \n"
  echo -ne "\033[33m- Input: \033[0m"
  read ZK_SERVERS
  ...
  echo "Start Mesos Marathon..."  && sleep 1
  $MESOS_MARATHON_BINARY \
  --master $ZK_SERVER/$ZK_ZNODE_PATH \
  --zk zk://$ZK_SERVER/$ZK_MARATHON_ZNODE_PATH \
  > $MESOS_MARATHON_LOGS 2>&1 &
  echo "done"
}
...
function f_kill_of_process {
  ...
  elif [[ "$ARG_2" == "ma" || "$ARG_2" == "marathon" ]]; then
      echo "Kill of Mesos Marathon..." && sleep 1
      kill -9 $MESOS_MARATHON_PID
      echo "done"
...
function f_help {
  ...
  echo "ma, marathon       : Start Mesos Marathon"
  ...
  echo
  echo "ma, marathon       : kill of marathon (k or kill option only.)"
  echo "                     ex) $ARG_0 k ma or $ARG_0 kill marathon"
  echo
}
